 ...
 * newline) using the temp-file-and-rename pattern (§8.5). The rename is
 * atomic on POSIX, so readers always see either the previous complete file
 ...
 * pattern (§8.5). See `atomicWriteJson` for full behaviour notes.
 */
