 ...
 *
 * Spawns Orchestrator.run()() in a child process (via child_process.fork). After step
 * "a" completes and step "b" starts streaming (signalled via IPC), the parent
 * sends SIGKILL to the child. The parent then calls Orchestrator.resume()(runDir) and
 * asserts:
 *
...
const CHILD_SCRIPT = join(HERE, 'fixtures', 'child-step.ts');
...
/**
 * Fork child-step.ts with --experimental-strip-types. Returns a promise that
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('exit', (code, signal) => {
      if (signalReceived) {
        ...
        settle(
          new Error(
            `child-step exited early without sending live-state-observed (code=${String(code)}, signal=${String(signal)})`,
          ),
        );
      }
    });
    ...
    const safetyTimer = setTimeout(() => {
      if (!signalReceived) {
        child.kill('SIGKILL');
        settle(new Error('child-step timed out waiting for live-state-observed IPC message'));
      }
    }, 20_000);
...
describe('crash-resume integration (real SIGKILL)', () => {
  ...
  it('zombie sweep fires, succeeded step is not re-invoked, final status is succeeded', {
    timeout: 30_000,
  }, async () => {
