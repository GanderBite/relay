...
describe('resume with complex flow topologies', () => {
  ...
  it('branch: resumes only the remaining B-path steps; A-path (skipped) is never invoked', {
    timeout: 20_000,
  }, async () => {
  ...
  it('parallel: resumes only the remaining child (c3) and downstream; c1/c2 are never re-invoked', {
    timeout: 20_000,
  }, async () => {
