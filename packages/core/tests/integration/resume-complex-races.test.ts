 ...
 *   parallel runner is marked succeeded here so the resume queue-seeder treats
...
/**
 * Write race-ref.json and metrics.json stubs so resume() can locate and
 * re-import the race and load cost data without encountering ENOENT.
 */
async function writeRunBoilerplate(
  ...
  await atomicWriteJson(join(runDir, 'race-ref.json'), {
    raceName,
    raceVersion,
    racePath,
  });
...
describe('resume with complex flow topologies', () => {
  ...
  it(
    'branch: resumes only the remaining B-path runners; A-path (skipped) is never invoked',
    { timeout: 20_000 },
    async () => {
      const racePath = join(FIXTURES_DIR, 'branch-resume-race.ts');
      const raceName = 'branch-resume-flow';
      const raceVersion = '0.1.0';
      ...
      await writeRunBoilerplate(runDir, raceName, raceVersion, racePath);
      ...
      const finalState = JSON.parse(finalStateRaw) as RaceState;
      expect(finalState.status).toBe('succeeded');
      expect(finalState.runners['b2']?.status).toBe('succeeded');
      expect(finalState.runners['end']?.status).toBe('succeeded');
      expect(finalState.runners['b1']?.status).toBe('succeeded');
      expect(finalState.runners['a1']?.status).toBe('skipped');
      expect(finalState.runners['a2']?.status).toBe('skipped');
    },
  );
  ...
  it(
    ...
    { timeout: 20_000 },
    async () => {
      const racePath = join(FIXTURES_DIR, 'parallel-resume-race.ts');
      const raceName = 'parallel-resume-flow';
      const raceVersion = '0.1.0';
      ...
      //
      // Marking the parallel runner as 'succeeded' is the state the run would
      ...
      // The parallel runner itself (kind: 'parallel') does not invoke the provider
      // directly, but assert it was not re-run to guard against resume re-seeding it.
      const finalStateRaw = await readFile(join(runDir, 'state.json'), 'utf8');
      const finalState = JSON.parse(finalStateRaw) as RaceState;
      expect(finalState.status).toBe('succeeded');
      expect(finalState.runners['c3']?.status).toBe('succeeded');
      expect(finalState.runners['end']?.status).toBe('succeeded');
      expect(finalState.runners['c1']?.status).toBe('succeeded');
      expect(finalState.runners['c2']?.status).toBe('succeeded');
      expect(finalState.runners['parallel']?.status).toBe('succeeded');
    },
  );
});
