...
describe('CostTracker', () => {
  ...
  it('[COST-002] summary aggregates totalUsd and totalTokens across steps', async () => {
    ...
    await tracker.record(
      metric({ stepId: 'a', model: 'sonnet', tokensIn: 100, tokensOut: 50, costUsd: 0.01 }),
    );
    await tracker.record(
      metric({ stepId: 'b', model: 'sonnet', tokensIn: 200, tokensOut: 100, costUsd: 0.02 }),
    );
    await tracker.record(
      metric({ stepId: 'c', model: 'sonnet', tokensIn: 50, tokensOut: 25, costUsd: 0.005 }),
    );
