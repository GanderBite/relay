...
import { CostTracker, type StepMetrics } from '../src/cost.js';
import { StateCorruptError } from '../src/errors.js';

function metric(partial: Partial<StepMetrics> & { stepId: string; model: string }): StepMetrics {
  return {
    flowName: 'f',
    runId: 'r',
...
describe('CostTracker', () => {
  ...
  it('[COST-001] record() appends and persists metrics.json atomically', async () => {
    ...
    const r1 = await tracker.record(
      metric({ stepId: 'a', model: 'sonnet', tokensIn: 10, tokensOut: 5, costUsd: 0.01 }),
    );
    const r2 = await tracker.record(
      metric({ stepId: 'b', model: 'sonnet', tokensIn: 20, tokensOut: 10, costUsd: 0.02 }),
    );
    expect(r1.isOk()).toBe(true);
    expect(r2.isOk()).toBe(true);
    const raw = await readFile(metricsPath, 'utf8');
    const arr = JSON.parse(raw) as StepMetrics[];
    expect(arr).toHaveLength(2);
    expect(arr[0].stepId).toBe('a');
    expect(arr[1].stepId).toBe('b');
  });

  it('[COST-002] summary aggregates totalUsd and totalTokens across steps', async () => {
    const tracker = new CostTracker(metricsPath);
    await tracker.record(metric({ stepId: 'a', model: 'sonnet', tokensIn: 100, tokensOut: 50, costUsd: 0.01 }));
    await tracker.record(metric({ stepId: 'b', model: 'sonnet', tokensIn: 200, tokensOut: 100, costUsd: 0.02 }));
    await tracker.record(metric({ stepId: 'c', model: 'sonnet', tokensIn: 50, tokensOut: 25, costUsd: 0.005 }));
  ...
  it('[COST-003] summary.perModel groups by model', async () => {
    const tracker = new CostTracker(metricsPath);
    await tracker.record(metric({ stepId: 'a', model: 'sonnet', costUsd: 0.01 }));
    await tracker.record(metric({ stepId: 'b', model: 'sonnet', costUsd: 0.02 }));
    await tracker.record(metric({ stepId: 'c', model: 'opus', costUsd: 0.05 }));
    const s = tracker.summary();
  ...
  it('[COST-004] load() rebuilds in-memory state from disk', async () => {
    ...
    await a.record(metric({ stepId: 'a', model: 'sonnet', tokensIn: 10, tokensOut: 5, costUsd: 0.01 }));
    await a.record(metric({ stepId: 'b', model: 'sonnet', tokensIn: 20, tokensOut: 10, costUsd: 0.02 }));
    const sA = a.summary();
  ...
  it('[COST-005] undefined costUsd contributes 0 (never NaN)', async () => {
    const tracker = new CostTracker(metricsPath);
    await tracker.record(metric({ stepId: 'a', model: 'sonnet', costUsd: 0.01 }));
    await tracker.record(metric({ stepId: 'b', model: 'sonnet' /* no costUsd */ }));
    const s = tracker.summary();
  ...
  it('load() on corrupt metrics.json returns StateCorruptError', async () => {
