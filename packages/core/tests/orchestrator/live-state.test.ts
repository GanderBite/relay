...
describe('writeLiveState (sprint 5 task_39)', () => {
  ...
  it('[LIVE-001] atomically persists <runDir>/live/<runnerId>.json with the given payload', async () => {
  ...
  it('[LIVE-002] multiple writeLiveState calls produce a monotonically current file', async () => {
    const iso = new Date().toISOString();
    await writeLiveState(tmp, 'a', { status: 'running', attempt: 1, startedAt: iso, lastUpdateAt: iso, tokensSoFar: 0 });
    await writeLiveState(tmp, 'a', { status: 'running', attempt: 1, startedAt: iso, lastUpdateAt: iso, tokensSoFar: 100 });
    await writeLiveState(tmp, 'a', { status: 'running', attempt: 1, startedAt: iso, lastUpdateAt: iso, tokensSoFar: 200 });
  ...
  it('[LIVE-003] <runDir>/live/ is cleared at run start — pre-existing files are removed', async () => {
    // Sprint 5 task_39 says the Runner clears the live dir at run start.
    ...
    await writeFile(join(tmp, 'live', 'old-runner.json'), '{"stale":true}', 'utf8');

    // Dynamically import a clearLiveDir helper if present; otherwise the
    // Runner's own startup should handle it. For unit-level coverage we call
    ...
    if (typeof clear === 'function') {
      ...
      expect(entries).not.toContain('old-runner.json');
    } else {
