...
**The `relay` metadata block** is read by the CLI's `list`, `search`, and pre-run banner. It is not consumed by `@ganderbite/relay-core` itself — the step runner loads `flow.ts`, not `package.json`. All five keys (`displayName`, `tags`, `estimatedCostUsd`, `estimatedDurationMin`, `audience`) are required. Catalog lint reports an error if any are missing.
...
## Peer dependency on @ganderbite/relay-core

Flow packages declare `@ganderbite/relay-core` as a `peerDependency`, not a direct dependency:
...
This means the user's project (or the CLI host) provides the `@ganderbite/relay-core` instance — the flow does not bundle its own copy. The caret range (`^1.0.0`) pins to a compatible major. Do not use `*` or an open range; the catalog lint treats an unpinned peer as a warning.

When `@ganderbite/relay-core` reaches `1.0.0`, flows published against `^1.0.0` continue to work across minor and patch releases. A breaking change to `@ganderbite/relay-core`'s public API requires a new major and a coordinated catalog update. Until `@ganderbite/relay-core` is at `1.0.0`, use an exact or tilde range (`~0.x.y`) to avoid surprises on patch bumps during the pre-release period.
