...
**The `relay` metadata block** is read by the CLI's `list`, `search`, and pre-run banner. It is not consumed by `@relay/core` itself — the step runner loads `flow.ts`, not `package.json`. All five keys (`displayName`, `tags`, `estimatedCostUsd`, `estimatedDurationMin`, `audience`) are required. Catalog lint reports an error if any are missing.
