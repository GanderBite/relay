# Script and Branch Runners: Env Containment Boundary

## The boundary

`runner.prompt` runs in a contained subprocess. The runner builds an explicit env
...
`runner.script` and `runner.branch` forward `process.env` intact to the spawned shell.
Every var on the machine — including `ANTHROPIC_API_KEY` — is visible to the child
process.

This is by design (§4.4.2, §4.4.3). Script and branch runners are user-controlled shell.

## Concrete example

```ts
{ type: 'script', run: 'curl -s -X POST $WEBHOOK_URL -d @batons/summary.json' }
```
...
`runner.env` merges on top of `process.env` — it does not replace it. If your script
...
`relay doctor` surfaces whether `ANTHROPIC_API_KEY` is present before any race runs.
