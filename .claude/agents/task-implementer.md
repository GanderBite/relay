...
4. **Consult relevant skills.** The `relay-monorepo`, `claude-agent-sdk`, `flow-package-format`, and `billing-safety` skills load on demand. If the task touches their domain, read the SKILL.md.
...
- **Atomic writes** for any file another process reads (state.json, handoffs, metrics, live state). Use the `atomicWriteJson` helper from `@relay/core` once it exists.
...
- You don't write prompts or example flows (flow-author does).
